...
-   **本地 HTTPS 支持**: 内置自签名 SSL 证书，方便在任何环境中进行安全的远程测试。
...
当您第一次通过 `https://` URL 访问应用时，您的浏览器会显示一个**安全警告**（例如，“您的连接不是私密连接”或“NET::ERR_CERT_AUTHORITY_INVALID”）。

**这是完全正常且预期的行为。** 因为该应用使用的是一个“自签名”SSL 证书，它由您的开发环境自动生成，而不是由浏览器默认信任的证书颁发机构（如 Let's Encrypt）颁发。为了进行开发和测试，我们需要手动信任这个证书。

#### 解决方案：信任证书

请按照以下步骤操作（以 Chrome 浏览器为例，其他浏览器类似）：

1.  在警告页面上，找到并点击“**高级**”按钮。
2.  点击“**继续前往...（不安全）**”链接。

完成此操作后，浏览器将记住您的选择，并将该页面加载到您的本地 HTTPS 服务器上。现在，所有的功能（包括文件哈希和 API 调用）都将在一个安全的上下文中正常运行。
